                  ...
                  <tbody v-if="departments.length">
                    <tr v-for="(item, index) in departments" :key="item.id">
                      ...
                      <td>
                        ...
                        <a
                          href="#"
                          @click="deleteDepartment(item.id, item.name, index)"
...
export default {
  ...
  setup() {
    ...
    let getDepartment = (page = 1) => {
      ...
      adminApi
        .get(`/v1/dashboard/department?page=${page}&search=${search.value}`)
        .then((res) => {
          let l = res.data.data;
    ...
    function deleteDepartment(id, departmentName, index) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          adminApi
            .delete(`/v1/dashboard/department/${id}`)
            .then((res) => {
    ...
    function activationDepartment(id, departmentName, active, index) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          adminApi
            .get(`/v1/dashboard/activationDepartment/${id}`)
            .then((res) => {
              Swal.fire({
                icon: "success",
                title: `${
