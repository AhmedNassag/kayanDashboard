  ...
  setup() {
    ...
    function activationTax(id, active, index) {
      Swal.fire({
        title: `${
          active
            ? "هل انت متاكد من ايقاف التفعيل ؟"
            : "هل انت متاكد من التفعيل  ؟"
        } `,
          ...
          adminApi
            ...
            .then((res) => {
              Swal.fire({
                icon: "success",
                title: `${
                  active ? "تم التفعيل بنجاح" : "تم ايقاف التفعيل بنجاح"
                }`,
                showConfirmButton: false,
                timer: 1500,
              });
    ...
    function appTax(id, app, index) {
      Swal.fire({
        title: `${
          app
            ? "هل انت متاكد من حذف الضريبة من التطبيق ؟"
            : "هل انت متاكد من اضافة الضريبة الى التطبيق  ؟"
        } `,
        text: `لم تتمكن من التراجع عن هذا`,
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          adminApi
            .get(`/v1/dashboard/activationTaxApp/${id}`)
            .then((res) => {
              Swal.fire({
                icon: "success",
                title: `${
                  app
                    ? " تم حذف الضريبة من التطبيق بنجاح"
                    : "تمت اضافة الضريبة الى التطبيق بنجاح"
                }`,
                showConfirmButton: false,
                timer: 1500,
              });
