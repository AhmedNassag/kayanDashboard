...
export default {
  setup(props, context) {
    ...
    const know_us_way_store = inject("know_us_way_store");
    const data = reactive({
      nameExist: false,
    });
    const form = reactive({
    ...
    function getForm() {
      return {
        id: props.selectedKnowUsWay ? props.selectedKnowUsWay.id : null,
        name: form.name,
      };
    }
    function setForm() {
      v$.value.$reset();
      form.name = props.selectedKnowUsWay ? props.selectedKnowUsWay.name : "";
      data.nameExist = false;
    }
    //Watchers
    watch(
      () => {
        know_us_way_store.onFormShow;
      },
      (value) => {
        setForm();
      },
