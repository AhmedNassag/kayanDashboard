<template>
    <div :class="[
        ...
        <div class="content container-fluid">
                            ...
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-4 text-center">
                                        <router-link v-if="permission.includes('complaint create')"
                                            :to="{ name: 'createComplaint' }"
                                            class="btn btn-custom btn-warning h-50 mt-4">
                                            {{ $t("global.Add") }}
                                        </router-link>
                                        <a
                                ...
                                <table class="table mb-0" id="div">
                                    ...
                                    <tbody v-if="complaints.length">
                                        <tr v-for="(item, index) in complaints" :key="item.id">
                                            ...
                                            <td class="text-center" v-if="item.kind">{{ $t('dashboard.'+ item.kind) }}
                                            </td>
                                            <td class="text-center" v-else>{{ $t("global.Not Found") }}</td>
                                            ...
                                            <td class="text-center" v-else>{{ $t("global.Not Found") }}</td>
                                            <td class="text-center">{{ printDate(item.created_at) }}</td>
                                            <td class="text-center">
                                                <router-link :to="{
                                                    name: 'editComplaint',
                                                    params: { id: item.id },
                                                }" v-if="permission.includes('complaint edit')" class="btn btn-sm btn-success me-2">
                                                    {{ $t("global.Edit") }}
                                                    <i class="far fa-edit"></i>
                                                </router-link>
                                                <a href="#" @click="deleteComplaint(item.id, index)"
                                                ...
                                                <router-link :to="{
                                                    name: 'showComplaint',
                                                    params: { id: item.id },
                                                }" v-if="item.reply" class="btn btn-sm btn-secondary me-2">
                                                ...
                                                <router-link :to="{
                                                    name: 'replyComplaint',
                                                    params: { id: item.id },
                                                }" v-else class="btn btn-sm btn-info me-2">
                                                    {{ $t("global.Add Reply") }}
            ...
            <!-- /Table -->
            <!-- start Pagination -->
            <Pagination :data="complaintsPaginate" @pagination-change-page="getComplaint">
                <template #prev-nav>
...
export default {
    ...
    setup() {
        ...
        let getComplaint = (page = 1) => {
            ...
            adminApi
                .get(`/v1/dashboard/complaint?page=${page}&search=${search.value}&platform=${platform.value}&from_date=${from_date.value}&to_date=${to_date.value}`)
                .then((res) => {
                    let l = res.data.data;
