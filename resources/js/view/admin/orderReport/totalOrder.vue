        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{ $t('global.totalOrderReport') }}</h3>
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ $t('global.totalOrderReport') }}</li>
                        </ul>
                    </div>
                            ...
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-md-2 row justify-content-end">
                                        <router-link v-if="permission.includes('PurchaseInvoice create')"
                                                     :to="{name: 'createOrderDirect', params: {lang: locale || 'ar'}}"
                                                                    ...
                                                                    <div role="tabpanel" :id="'tab-4-'+item.id" class="tab-pane fade active show">
                                                                        ...
                                                                        <div class="row justify-content-between">
                                                                            ...
                                                                            <div class="col-4 d-flex w-25 justify-content-end">
                                                                                <router-link @click="close(item.id)" v-if="permission.includes('PurchaseInvoice edit') && item.order_status == 0"
                                                                                             :to="{name: 'editOrderDirect', params: {id:item.id}}"
                                                                                             class="btn btn-sm btn-success  head-button">
                                                                        ...
                                                                        <div class="table-responsive" :id="'printData-'+item.id">
                                                                                    ...
                                                                                    <p>
                                                                                        ...
                                                                                        {{
                                                                                            item.order_other_offer?
                                                                                                parseFloat( item.subTotal - item.order_other_offer.offer - item.discount_offer) :
                                                                                                parseFloat( item.subTotal - item.discount_offer)
                                                                                        }}
                                                                                        {{ item.currency }}
                                                                                    </p>
                                                                                    <p>{{$t('global.taxValue')}} : {{ item.tax }} {{ item.currency }}</p>
                                                                                    <p>{{$t('global.TotalPriceAfterTax')}} : {{ parseFloat(item.total- item.shippingPrice) }} {{ item.currency }}</p>
                                                                                    <p>{{$t('global.shipping')}} : {{ item.shippingPrice }} {{ item.currency }}</p>
                                                                            ...
                                                                            <table class="table table-center table-hover mb-0 datatable">
                                                                                ...
                                                                                <tbody v-if="item.order_store_product.length">
                                                                                <tr v-for="(it,index) in item.order_store_product" :key="it.id">
                                                                                    <td>{{ index +1}}</td>
                                                                                    <td>{{ it.store_product.product.name }}</td>
                                                                                    <td>{{ it.amount }} ( {{it.measurement_unit.name}} )</td>
                                                                                    <td>{{ it.product_price}}</td>
                                                                                    <td>{{ dateFormat(it.store_product.expiry_date) }}</td>
                                                                                    <td>{{ parseFloat(it.amount * it.product_price) }}</td>
                                                                                </tr>
                                                                                </tbody>
                                                                                <tbody  v-else>
                                                                                <tr>
...
export default {
    ...
    setup() {
        ...
        let getOrder = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/orderDirect?page=${page}&from_date=${fromDate.value}&to_date=${toDate.value}&status=5`)
                .then((res) => {
        ...
        return {ordersPaginate,permission,order_id,fromDate,toDate,orders, loading,dateFormat,printData, getOrder,close};
