        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{$t('global.Update Mobile Banners')}}</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><router-link :to="{name: 'dashboard', params: {lang: locale || 'ar'}}">{{$t('dashboard.Dashboard')}}</router-link></li>
                            <li class="breadcrumb-item active">{{$t('global.Update Mobile Banners')}}</li>
                        </ul>
                    </div>
...
export default {
    ...
    setup(){
        ...
        let getBanners = () => {
            loading.value = true;
            adminApi.get(`/v1/dashboard/get_mobile_banners`)
                .then((res) => {
        ...
        const changeStatus =async (banner_id) => {
            await adminApi.post('/v1/dashboard/change_mobile_banner_status',{'banner_id':banner_id})
        }
