...
export default {
  setup(props, context) {
    ...
    function save() {
      ...
      insertBestSellerSettings();
    }
    //Commons
    function getBestSellerSettings() {
      context.emit("loading", true);
      bestSellerClient.getBestSellerSettings().then((response) => {
        context.emit("loading", false);
        data.bestSellerSettings = response.data;
        form.limit = data.bestSellerSettings ? data.bestSellerSettings.limit : 2;
      });
    }
    function insertBestSellerSettings() {
      context.emit("loading", true);
      bestSellerClient
        .insertBestSellerSettings(getForm())
        .then((response) => {
          context.emit("loading", false);
