                ...
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ $t('global.supplierExpenses') }}</li>
                        </ul>
                    </div>
                            ...
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-md-2 row justify-content-end">
                                        <router-link
                                            :to="{name: 'createSupplierExpenses'}"
                                            v-if="permission.includes('supplierExpenses create')"
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody v-if="purchases.length">
                                        <tr v-for="(item,index) in purchases"  :key="item.id">
                                            ...
                                            <td>
                                                <router-link
                                                    :to="{name: 'editSupplierExpenses', params: {id:item.id}}"
                                                    v-if="permission.includes('supplierExpenses edit') && item.supplier_account"
                                                    class="btn btn-sm btn-success me-2">
                                                    <i class="far fa-edit"></i>
                                                </router-link>
                                                <a href="#" @click="deleteJob(item.id,index)"
                                                   v-if="permission.includes('supplierExpenses delete') && item.supplier_account"
                                                                ...
                                                                <div class="row">
                                                                    <div class="form-group col-lg-12 col-md-12">
                                                                        <h5>{{ $t('global.SendToMr') }} :
                                                                            {{ item.supplier.name_supplier }}</h5>
                                                                    </div>
...
export default {
    ...
    setup() {
        ...
        let getIncome = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/supplierExpense?page=${page}&search=${search.value}&from_date=${fromDate.value}&to_date=${toDate.value}`)
                .then((res) => {
                    let l = res.data.data;
        ...
        function deleteJob(id, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/supplierExpense/${id}`)
                        .then((res) => {
