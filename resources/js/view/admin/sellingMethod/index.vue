<template>
  ...
  >
    <div class="content container-fluid">
        ...
        <div class="row align-items-center">
          <div class="col">
            ...
            <ul class="breadcrumb">
              ...
              <li class="breadcrumb-item active">{{ $t("global.Selling Methods") }}</li>
            </ul>
          </div>
        </div>
      </div>
                  ...
                  <tbody v-if="sellingMethods.length">
                    <tr v-for="(item, index) in sellingMethods" :key="item.id">
                      ...
                      <td>
                        <a
                          href="#"
                          @click="activationSellingMethod(item.id, item.status, index)"
                        >
                          <span
                            :class="[
                      ...
                      <td>
                        ...
                        <a
                          href="#"
                          @click="deleteSellingMethod(item.id, index)"
      ...
      <!-- start Pagination -->
      <Pagination
        :data="sellingMethodsPaginate"
        @pagination-change-page="getSellingMethod"
      >
        <template #prev-nav>
...
export default {
  ...
  setup() {
    ...
    let getSellingMethod = (page = 1) => {
      ...
      adminApi
        .get(`/v1/dashboard/sellingMethod?page=${page}&search=${search.value}`)
        .then((res) => {
          let l = res.data.data;
    ...
    function deleteSellingMethod(id, index) {
      Swal.fire({
    ...
    function activationSellingMethod(id, active, index) {
      Swal.fire({
        title: `${
          active
        ...
        if (result.isConfirmed) {
          adminApi
            .get(`/v1/dashboard/activationSellingMethod/${id}`)
            .then((res) => {
              Swal.fire({
                icon: "success",
                title: `${
              ...
              sellingMethods.value[index]["status"] = active ? 0 : 1;
            })
            .catch((err) => {
              Swal.fire({
                icon: "error",
    ...
    return {
      ...
      search,
      deleteSellingMethod,
      activationSellingMethod,
      sellingMethodsPaginate,
      sellingMethods,
    };
  },
  data() {
