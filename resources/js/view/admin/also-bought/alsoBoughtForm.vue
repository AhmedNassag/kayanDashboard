...
export default {
  setup(props, context) {
    ...
    const also_bought_store = inject("also_bought_store");
    const form = reactive({
      id: null,
      product_id: null,
    });
    ...
    function save() {
      ...
      if (!props.selectedAlsoBoughtProduct) {
        store();
      } else {
        update();
      }
    ...
    function store() {
      ...
      alsoBoughtClient
        ...
        .then((response) => {
          ...
          $("#alsoBoughtFormModal #close-button").click();
    ...
    function getForm() {
      return {
        id: props.selectedAlsoBoughtProduct ? props.selectedAlsoBoughtProduct.id : null,
        product_id: form.product_id,
      };
    }
    function setForm() {
      v$.value.$reset();
      form.product_id = props.selectedAlsoBoughtProduct
        ? props.selectedAlsoBoughtProduct.product_id
        : null;
    }
    //Watchers
    watch(
      () => {
        also_bought_store.onFormShow;
      },
      (value) => {
        console.log(props.selectedAlsoBoughtProduct);
        setForm();
      },
      { deep: true }
    );
  ...
  props: ["selectedAlsoBoughtProduct", "products"],
};
</script>
